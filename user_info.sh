...
#This script can be used to print information relevant to the current user, user session and shell information.
...
echo -e "This script's current shell executable is: echo -e ${RED}${SHELL}${NC}" #echo tack-"e" is used to allow backslash escapes #WORKING!!!
echo ""

echo -e "The Current PID of this script being executed is: ${YELLOW}{$$}${NC}"
#echo -e ""${YELLOW}"$$"${NC}""
echo ""
...
echo -e ${YELLOW}${RESULT6}${NC}${RESULT7}${RED}${RESULT4}${NC}
echo ""
echo ""
####################################################################################################

#echo -e "Printing basic user id information for "${GREEN}"${USER}"${NC}"..."
echo -e "Printing basic user id information for "${GREEN}"${USER}"${NC}"..."
#id
#RESULT8=`echo -e ${USER} | grep ${USER}`
#RESULT9=`echo -e ${RESULT8} | grep ${USER}`
#for user in ${RESULT9}
#  do
#    $(echo -e ${RESULT9} | grep ${USER})
...
EUIDVAR=`id -u > /tmp/EUIDVAR.tmp && cat /tmp/EUIDVAR.tmp`
#EUIDVAR2=$(cat /tmp/EUIDVAR.tmp)
#echo ${EUIDVAR2}
echo -e "${EUIDVAR} is the EUID (Effective User ID)."
